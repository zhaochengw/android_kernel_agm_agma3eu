...
extern int is_otg_enabled;
static int otg_oc;
static bool check_soc_flag;
static bool first_check_profile;
...
struct smb1360_chip {
	...
	bool		probe_done;
	bool		fg_reset_flag;
	/* to make sure enable/disable fg access appears pairly */
	bool		fg_access_ok;
	...
	struct qpnp_vadc_chip		*qpnp_vadc_dev;
	struct wake_lock        vbat_present_wake_lock;
	...
	unsigned int			parallel_current_levels;
	unsigned int			*parallel_current_mitigation;
...
static int smb1360_enable_fg_access(struct smb1360_chip *chip)
{
	...
	while (timeout) {
		...
		pr_debug("smb1360_enable_fg_access IRQ_I_REG =%d\n", reg);
		if (rc)
	...
	if (timeout <= 40) {
		rc = smb1360_read(chip, CMD_I2C_REG, &reg2);
		if (!rc)
			pr_err("timeout %d 0x%x:0x%x 0x%x:0x%x\n",
				timeout, IRQ_I_REG, reg, CMD_I2C_REG, reg2);
	}
	pr_debug("timeout=%d\n", timeout);
...
static int smb1360_get_prop_voltage_now(struct smb1360_chip *chip)
{
	...
	if (chip->batt_present) {
		...
		chip->voltage_now = 4000 * 1000;

		pr_info("Vbat is not present report fake voltage\n");
	}
	return chip->voltage_now;
}
...
static int smb1360_set_appropriate_usb_current(struct smb1360_chip *chip)
{
	...
	if (current_ma <= CURRENT_100_MA) {
		...
		if (i < 0) {
			...
			/* Fast CHG Current can be a little bigger */
			i++;
		}
...
static int battery_missing_handler(struct smb1360_chip *chip, u8 rt_stat)
{
	...
	if (chip->batt_present
		&& (wake_lock_active(&chip->vbat_present_wake_lock))) {
			wake_unlock(&chip->vbat_present_wake_lock);
	} else if (!chip->batt_present
		&& !(wake_lock_active(&chip->vbat_present_wake_lock))) {
			wake_lock(&chip->vbat_present_wake_lock);
	}
	return 0;
}
...
#ifdef	CONFIG_HISENSE_INTELLIGENT_CHARGE
static void smb1360_intelligent_charge_disable(struct smb1360_chip *chip)
{
	union power_supply_propval ret = {0, };

	if (!chip->thermal_flag)
		return;
...
#ifdef	CONFIG_HISENSE_SCALE_BOARD_THERM
static void smb_parallel_intelligent_chg_disable(struct smb1360_chip *chip)
{
	union power_supply_propval prop = {0, };

	if (!chip->parallel_therm_flag)
		return;

	chip->parallel_therm_flag = 0;
	prop.intval = 0;
	chip->max_parallel_chg_current =
		chip->parallel_current_mitigation[0];
	smb1360_parallel_charger_enable(chip,
					PARALLEL_CURRENT, false);
	chip->batt_psy.set_property(&chip->batt_psy,
...
static int usbin_uv_handler(struct smb1360_chip *chip, u8 rt_stat)
{
	...
	if (chip->usb_present && !usb_present) {
		...
		chip->usb_present = usb_present;
#ifdef	CONFIG_HISENSE_SCALE_BOARD_THERM
	smb_parallel_intelligent_chg_disable(chip);
#endif /*CONFIG_HISENSE_SCALE_BOARD_THERM*/

#ifdef	CONFIG_HISENSE_INTELLIGENT_CHARGE
	smb1360_intelligent_charge_disable(chip);
#endif /*CONFIG_HISENSE_INTELLIGENT_CHARGE*/
...
static int smb1360_switch_to_profile_b(struct smb1360_chip *chip)
{
	...
	reg = !!(reg & BATTERY_PROFILE_BIT);
	if (reg == new_profile) {
		pr_info("Profile B=%d already loaded\n", new_profile);
		return 0;
	}
...
static int determine_initial_status(struct smb1360_chip *chip)
{
	...
	pr_info("smb determine_initial_status, set usb status\n");
...
static void smb_otg_reconnect_work(struct work_struct *work)
{
	...
	pr_info("@@@@@@clear OTG Enable bit after OC\n");
	rc = smb1360_masked_write(chip, CMD_CHG_REG, CMD_OTG_EN_BIT, 0);
	if (rc)
		pr_info("cannot clear OTG Enable bit after OC\n");
	pr_info("@@@@@@set OTG Enable bit after OC\n");
	rc = smb1360_masked_write(chip, CMD_CHG_REG,
...
static int smb1360_fg_config(struct smb1360_chip *chip, bool init)
{
	int rc = 0, temp, fcc_mah;
	u8 reg = 0, reg2[2];

	if (chip->fg_reset_at_pon && init) {
...
static int smb1360_enable(struct smb1360_chip *chip, bool enable)
{
	...
	pr_info("pre smb SHDN_CTRL_REG is 0x%x\n", val);
	...
	smb1360_read(chip, CMD_IL_REG, &val);

	pr_info("aft smb CMD_IL_REG is 0x%x\n", val);
	...
	if (chip->otp_csir_2243) {
		...
		if (chip->batt_capacity_mah > CSIR2289_SWITCH_CAPACITY) {
			if (chip->batt_capacity_mah > CSIR2289_MAX_CAPACITY)
				pr_err("CSIR2289 can't cover %d mAh\n",
...
static int smb1360_csir_predicate(struct smb1360_chip *chip)
{
	...
	if (reg == OTP_CSIR_2243)
		chip->otp_csir_2243 = true;
	else
		chip->otp_csir_2243 = false;
...
static int smb1360_hw_init(struct smb1360_chip *chip)
{
	...
	if (chip->safety_time != -EINVAL) {
		if (chip->safety_time == 0) {
			...
			rc = smb1360_masked_write(chip,
				CFG_SFY_TIMER_CTRL_REG,
				SAFETY_TIME_DISABLE_BIT
				|SAFETY_TIME_EN_BIT
				|SAFETY_TIME_MINUTES_MASK,
				SAFETY_TIME_EN_BIT
				| reg);
			if (rc < 0) {
				dev_err(chip->dev,
	...
	if (chip->otg_batt_uvlo_thresh != -EINVAL) {
		...
		i += 2;
		pr_info("OTG UVLO threshold i = %d\n", i);
	...
	rc = smb1360_fg_config(chip, true);
	if (rc < 0) {
...
static int smb1360_parse_parallel_charging_params(struct smb1360_chip *chip)
{
	...
	if (of_property_read_bool(node, "qcom,parallel-charging-enabled")) {
		...
		if (of_find_property(node, "qcom,parallel-current-mitigation",
					&chip->parallel_current_levels)) {
			chip->parallel_current_mitigation =
				devm_kzalloc(chip->dev,
				chip->parallel_current_levels, GFP_KERNEL);

			if (chip->parallel_current_mitigation == NULL) {
				pr_err("parallel current mitigation kzalloc() failed.\n");
				return -ENOMEM;
			}

			chip->parallel_current_levels /= sizeof(int);
			rc = of_property_read_u32_array(node,
					"qcom,parallel-current-mitigation",
					chip->parallel_current_mitigation,
					chip->parallel_current_levels);
			if (rc) {
				pr_err("Couldn't read parallel limits rc = %d\n",
					rc);
				return rc;
			}
		}
...
#if 0
	chip->qpnp_vadc_dev = qpnp_get_vadc(chip->dev, "qpnp");
	if (IS_ERR(chip->qpnp_vadc_dev)) {
		rc = PTR_ERR(chip->qpnp_vadc_dev);
		if (rc == -EPROBE_DEFER) {
...
#ifdef	CONFIG_HISENSE_INTELLIGENT_CHARGE
	rc = of_property_read_string(node, "qcom,cpu0-sensor-name",
							&sensor_type);
	if (rc)
		pr_err("Can't read tsens name\n");

	chip->sensor_id = sensor_get_id((char *)sensor_type);

	rc = of_property_read_u32(node, "qcom,cpu0-temp-max", &chip->temp_max);
	if (rc < 0)
		chip->temp_max = -EINVAL;

	rc = of_property_read_u32(node, "qcom,cpu0-temp-min", &chip->temp_min);
	if (rc < 0)
		chip->temp_min = -EINVAL;
	pr_info("temp_max = %d, temp_min = %d\n",
		chip->temp_max, chip->temp_min);
#endif /*CONFIG_HISENSE_INTELLIGENT_CHARGE*/

#ifdef	CONFIG_HISENSE_SCALE_BOARD_THERM
	rc = of_property_read_u32(node,
		"qcom,board-temp-max", &chip->board_temp_max);
	if (rc < 0)
		chip->board_temp_max = -EINVAL;

	rc = of_property_read_u32(node,
		"qcom,board-temp-min", &chip->board_temp_min);
	if (rc < 0)
		chip->board_temp_min = -EINVAL;

	pr_err("board_temp_max = %d, board_temp_min = %d\n",
		chip->board_temp_max, chip->board_temp_min);
#endif /*CONFIG_HISENSE_SCALE_BOARD_THERM*/
...
#ifdef CONFIG_HISENSE_INTELLIGENT_CHARGE
static void smb1360_intelligent_charge_enable(struct smb1360_chip *chip)
{
	int rc;
	union power_supply_propval ret = {0, };
	...
	if (chip->sensor_id < 0) {
		pr_err("sensor_id is error\n");
		return;
	}

	if ((chip->temp_max >= 0) && (chip->temp_min >= 0)) {
		rc = sensor_get_temp(chip->sensor_id, &chip->cpu0_temp);
		if (rc)
			pr_err("Unable to read TSENS sensor:%d. err:%d\n",
			chip->sensor_id, rc);

		pr_info("cpu0 temp is =%ld, thermal_flag=%d\n",
			chip->cpu0_temp, chip->thermal_flag);

		if (chip->thermal_flag == 0) {
			if ((chip->cpu0_temp >= chip->temp_max)
				&& (chip->up_count < 3))
				chip->up_count++;
			else
				chip->up_count = 0;
		...
		} else if (chip->thermal_flag) {
			if ((chip->cpu0_temp <= chip->temp_min)
				&& (chip->down_count < 3))
				chip->down_count++;
			else
				chip->down_count = 0;

			if (chip->down_count == 3) {
				ret.intval = 0;
				chip->batt_psy.set_property(&chip->batt_psy,
					POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL,
					&ret);
...
#ifdef	CONFIG_HISENSE_SCALE_BOARD_THERM
static void smb_parallel_intelligent_chg_enable(struct smb1360_chip *chip)
{
	int rc;
	static int i;
	struct power_supply *parallel_psy = get_parallel_psy(chip);
	...
	if (rc < 0) {
		pr_err("could not get board_temp, rc = %d", rc);
		return;
	}

	pr_info("board temp is %d", prop.intval);
	if ((prop.intval >= chip->board_temp_max)
		&& !chip->parallel_therm_flag) {
		i = 1;
		prop.intval = i;
		chip->max_parallel_chg_current =
			chip->parallel_current_mitigation[i];
		smb1360_parallel_charger_enable(chip,
					PARALLEL_CURRENT, false);
		chip->batt_psy.set_property(&chip->batt_psy,
			POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL, &prop);
		chip->parallel_therm_flag = 1;
	} else if ((prop.intval <= chip->board_temp_min)
		&& chip->parallel_therm_flag) {
		i = 0;
		prop.intval = i;
		chip->max_parallel_chg_current =
	...
	pr_info("max_parallel_chg_current=%d, chip->thermal_mitigation[%d]=%d\n",
			chip->max_parallel_chg_current,
			i, chip->thermal_mitigation[i]);
}
#endif /*CONFIG_HISENSE_SCALE_BOARD_THERM*/

static void smb_fg_reset(struct smb1360_chip *chip)
{
	int voltage, curr_val, rc;
	...
	if (!check_soc_flag && chip->batt_present) {
		check_soc_flag = true;
		bat_resist = smb1360_get_prop_batt_resistance(chip);
		bat_ocv = voltage + (curr_val/1000) * (bat_resist/1000);
		bat_ocv = bat_ocv/1000;

		pr_err("voltage = %d,curr_val = %d,bat_resist = %d,bat_ocv=%d\n",
			voltage/1000, curr_val/1000, bat_resist/1000, bat_ocv);

		if (chip->fg_reset_flag || ((UI_SOC < 15)
			&& abs(bat_ocv - vbat_for_first)
			> delta_vol_threshold)) {
			pr_info("FG RESET WORK\n");
			rc = smb1360_enable_fg_access(chip);
			if (rc)
				pr_err("Couldn't enable FG access rc=%d\n", rc);
			msleep(1500);
...
disable_fg_reset:
			smb1360_disable_fg_access(chip);
			msleep(1500);
			rc = smb1360_fg_config(chip, false);
			if (rc < 0)
				pr_err("%s fg config failed\n", __func__);
			UI_SOC = smb1360_get_prop_batt_capacity(chip);
...
static void smb_switch_profile(struct smb1360_chip *chip)
{
	int	rc;

	if (smb1360_should_swith_to_profile_b(chip)) {
		pr_info("%s:select profile B\n", __func__);
...
static void smb_capacity_work(struct work_struct *work)
{
	...
	if (!chip->batt_present && (voltage < (chip->v_cutoff_mv * 1000))) {
		vbat_is_dead = true;
		power_supply_changed(&chip->batt_psy);
	}
	...
	if (first_check_profile)
		smb_switch_profile(chip);

	if (stop_charge_expired != timeout_set_flag) {
		smb1360_masked_write(chip, CFG_SFY_TIMER_CTRL_REG,
			SAFETY_TIME_DISABLE_BIT,
			(stop_charge_expired ? 0 : SAFETY_TIME_DISABLE_BIT));
		timeout_set_flag = stop_charge_expired;
	}

#ifdef	CONFIG_HISENSE_INTELLIGENT_CHARGE
		smb1360_intelligent_charge_enable(chip);
#endif	/*CONFIG_HISENSE_INTELLIGENT_CHARGE*/

#ifdef	CONFIG_HISENSE_SCALE_BOARD_THERM
		smb_parallel_intelligent_chg_enable(chip);
#endif /*CONFIG_HISENSE_SCALE_BOARD_THERM*/
				...
				const struct i2c_device_id *id)
{
	...
	chip->fg_access_ok = false;
	chip->fg_reset_flag = false;
	chip->input_current_value = 0;
	chip->soc_calibration_flag = false;
	first_check_profile = true;
	...
	/*The gpio default direction is input.
		Add hisense code,just ensure the gpio direction is input,
		so even it happens error,it needn't return.
	*/
	if (gpio_is_valid(chip->irq_gpio)) {
		rc = gpio_request(chip->irq_gpio, "smb1360_irq_gpio");
		if (rc)
	...
	wake_lock_init(&chip->usbin_irq_wake_lock, WAKE_LOCK_SUSPEND,
				"usbin_irq_lock");
	wake_lock_init(&chip->vbat_present_wake_lock, WAKE_LOCK_SUSPEND,
				"vbat_present_lock");
	...
	if (!chip->batt_present) {
		vbat_val = smb1360_get_prop_voltage_now(chip);
		if (vbat_val <= 0)
			pr_err("Couldn't read vbat voltage\n");
		else if (vbat_val < (chip->v_cutoff_mv * 1000)) {
			vbat_is_dead = true;
			power_supply_changed(&chip->batt_psy);
		} else {
			wake_lock(&chip->vbat_present_wake_lock);
	...
	dev_info(chip->dev,
			"SMB1360 revision=0x%x csir2243 %d 10mOhm %d probe success! batt=%d usb=%d soc=%d\n",
			chip->revision, chip->otp_csir_2243,
			chip->rsense_10mohm,
...
static int smb1360_resume(struct device *dev)
{
	...
	schedule_delayed_work(&chip->capacity_work, 0);
...
static void smb1360_shutdown(struct i2c_client *client)
{
	int rc;
	struct smb1360_chip *chip = i2c_get_clientdata(client);
	smb1360_charging_disable(chip, USER, false);/* force enable charging */
	rc = smb1360_otg_disable(chip);
	if (rc)
		pr_err("Couldn't disable OTG mode rc=%d\n", rc);
