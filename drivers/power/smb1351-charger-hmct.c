...
#define SWITCH_FREQ_750K			00
#define SWITCH_FREQ_1M			    0x40
...
#define SMB_VERSION_REG          0x2E
#define SMB_VERSION_BIT                 BIT(1)
						...
						int present)
{
	...
	if (present) {
		...
		reg = SWITCH_FREQ_750K;
		rc = smb1351_masked_write(chip, OTG_TLIM_CTRL_REG,
				SWITCH_FREQ_MASK, reg);
		if (rc) {
			pr_err("Couldn't set OTG_TLIM_CTRL_REG rc=%d\n", rc);
			return rc;
		}

		reg = INPUT_MISSING_POLLER_CONFIG_BIT;
		rc = smb1351_masked_write(chip, PON_OPTIONS_REG,
				INPUT_MISSING_POLLER_CONFIG_BIT, reg);
		if (rc) {
			pr_err("Couldn't set PON_OPTIONS_REG rc=%d\n", rc);
			return rc;
		}
		...
		reg = USB_2_3_MODE_SEL_BY_I2C | USB_CMD_POLARITY_500_1_100_0;
		rc = smb1351_masked_write(chip, CHG_OTH_CURRENT_CTRL_REG,
				USB_2_3_MODE_SEL_BIT |
				USB_5_1_CMD_POLARITY_BIT, reg);
		if (rc) {
			pr_err("Couldn't set CHG_OTH_CURRENT_CTRL_REG rc=%d\n", rc);
			return rc;
		}
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		if (chip->parallel_charger_present) {
			chip->charging_disabled_status = !val->intval;
