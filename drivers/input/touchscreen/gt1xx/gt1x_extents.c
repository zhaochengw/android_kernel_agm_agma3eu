...
st_gesture_data gesture_data; /* gesture data buffer */
	...
	const char __user *buff, size_t len, loff_t *off)
{
	...
	ret = copy_from_user(&data->gesture_enabled, buff, 1);
	if (ret) {
		GTP_ERROR("copy_from_user failed.");
		return -EPERM;
	}

	GTP_DEBUG("gesture enabled:%x, ret:%d", data->gesture_enabled, ret);
...
*/
	...
	for (i = 0; i < ts->pdata->gesture_key->nbuttons; i++)
		if (ges_type == ts->pdata->gesture_key->map[i])
			break;
	...
	if(i >= ts->pdata->gesture_key->nbuttons)
		...
		if((ts->gesture_state) >> (i + 1) & 1) {
			input_report_key(dev, ts->pdata->gesture_map->map[i], 1);
			input_sync(dev);
			input_report_key(dev, ts->pdata->gesture_map->map[i], 0);
			input_sync(dev);
		} else {
...
#endif /* CONFIG_TOUCHSCREEN_GT1X_GESTURE */
...
static s32 hotknot_block_rw(u8 rqst_hotknot_state, s32 wait_hotknot_timeout)
{
	...
	if (wait_hotknot_timeout <= 0)
		...
		wait_event_interruptible_timeout(bp_waiter,
			force_wake_flag || rqst_hotknot_state ==
				(got_hotknot_state & rqst_hotknot_state),
...
s32 hotknot_event_handler(u8 *data)
{
	...
	if (hotknot_paired_flag) {
		...
		GTP_DEBUG("[0x8800~0x8803]=0x%x,0x%x,0x%x,0x%x",
			hn_state_buf[0], hn_state_buf[1],
				hn_state_buf[2], hn_state_buf[3]);

		if (wait_hotknot_state & HN_MASTER_SEND) {
			if ((0x03 == hn_state_buf[0])
				|| (0x04 == hn_state_buf[0])
				|| (0x07 == hn_state_buf[0])) {
...
#define GESTURE_DATA_OBTAIN		 (_IOR(GOODIX_MAGIC_NUMBER, 6, u8)\
							& NEGLECT_SIZE_MASK)
...
#define HOTKNOT_VENDOR_VERSION	  (_IOR(GOODIX_MAGIC_NUMBER, 28, u8)\
							& NEGLECT_SIZE_MASK)
#define HOTKNOT_WAKEUP_BLOCK _IO(GOODIX_MAGIC_NUMBER, 29)

#define IO_IIC_READ		(_IOR(GOODIX_MAGIC_NUMBER, 100, u8)\
							& NEGLECT_SIZE_MASK)
#define IO_IIC_WRITE	(_IOW(GOODIX_MAGIC_NUMBER, 101, u8)\
							& NEGLECT_SIZE_MASK)
...
#define IO_GET_VERISON	(_IOR(GOODIX_MAGIC_NUMBER, 110, u8)\
						& NEGLECT_SIZE_MASK)
#define IO_PRINT		(_IOW(GOODIX_MAGIC_NUMBER, 111, u8)\
						& NEGLECT_SIZE_MASK)
...
static long gt1x_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (_IOC_DIR(cmd)) {
		if (_IOC_DIR(cmd) & _IOC_WRITE) {
			err = copy_from_user(data,
				(void __user *)arg, data_length);
			if (err) {
				GTP_ERROR("Can't access the memory.");
				kfree(data);
				return -EFAULT;
	...
	case IO_ENABLE_IRQ:
		...
		/* print a string to syc log messages
	...
	case GESTURE_DATA_OBTAIN:
		...
		mutex_lock(&gesture_data_mutex);
		ret = copy_to_user(((u8 __user *) arg),
			&gesture_data.data, 4 + gesture_data.data[1] * 4
			+ gesture_data.data[3]);
		if (ret) {
	...
	case HOTKNOT_VENDOR_VERSION:
		ret =  copy_to_user(((u8 __user *) arg), HOTKNOT_VERSION,
			sizeof(HOTKNOT_VERSION));
		if (!ret)
...
#ifdef CONFIG_COMPAT
static long gt1x_compat_ioctl(struct file *file,
