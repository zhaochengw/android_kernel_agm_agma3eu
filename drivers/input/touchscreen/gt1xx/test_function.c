...
static s32 _save_testing_data(char *save_test_data_dir, int test_types)
{
	...
	if (current_data_index == 0) {
		bytes = (s32) snprintf((char *)data, PAGE_SIZE, "Config:\n");
		for (i = 0; i < sys.config_length; i++) {
			bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "0x%02X,", module_cfg[i]);
		}
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "\n\n");
		...
		if ((test_types & _MAX_CHECK) != 0) {
			bytes = (s32) snprintf((char *)data, PAGE_SIZE, "Channel maximum:\n");
			for (i = 0; i < sys.sc_sensor_num; i++) {
				for (j = 0; j < sys.sc_driver_num; j++) {
					bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "%d,", max_limit_value[i + j * sys.sc_sensor_num]);
				}
				bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "\n");
				ret = fwrite(data, bytes, 1, fp);
				bytes = 0;
			...
			for (i = 0; i < sys.key_number; i++) {
				...
				bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "%d,", max_key_limit_value[j++]);
		...
		if ((test_types & _MIN_CHECK) != 0) {
			bytes = (s32) snprintf((char *)data, PAGE_SIZE, "Channel minimum:\n");
			for (i = 0; i < sys.sc_sensor_num; i++) {
				for (j = 0; j < sys.sc_driver_num; j++) {
					bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "%d,", min_limit_value[i + j * sys.sc_sensor_num]);
				}
				bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "\n");
				ret = fwrite(data, bytes, 1, fp);
				bytes = 0;
			...
			for (i = 0; i < sys.key_number; i++) {
				...
				bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "%d,", min_key_limit_value[j++]);
			}
			bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "\n");
			ret = fwrite(data, bytes, 1, fp);
			if (ret < 0) {
		...
		if ((test_types & _ACCORD_CHECK) != 0) {
			bytes = (s32) snprintf((char *)data, PAGE_SIZE, "Channel average:(%d)\n", FLOAT_AMPLIFIER);
			for (i = 0; i < sys.sc_sensor_num; i++) {
				for (j = 0; j < sys.sc_driver_num; j++) {
					bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "%ld,", accord_limit[i + j * sys.sc_sensor_num]);
				}
				bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "\n");
				ret = fwrite(data, bytes, 1, fp);
				bytes = 0;
			...
			bytes = (s32) snprintf((char *)data, PAGE_SIZE, "\n");
			ret = fwrite(data, bytes, 1, fp);
			if (ret < 0) {
				WARNING("write to file fail.");
				goto exit_save_testing_data;
			...
			if (sys.chip_type == _GT1143 || sys.chip_type == _GT1133) {
				bytes = (s32) snprintf((char *)data, PAGE_SIZE, "Channel line accord:(%d)\n", FLOAT_AMPLIFIER);
				for (i = 0; i < sys.sc_sensor_num; i++) {
					for (j = 0; j < sys.sc_driver_num; j++) {
						bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "%ld,", accord_line_limit[i + j * sys.sc_sensor_num]);
					}
					bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "\n");
					ret = fwrite(data, bytes, 1, fp);
					bytes = 0;
		...
		bytes = (s32) snprintf((char *)data, PAGE_SIZE, " Rawdata\n");
		ret = fwrite(data, bytes, 1, fp);
		if (ret < 0) {
			WARNING("write to file fail.");
			goto exit_save_testing_data;
	...
	bytes = (s32) snprintf((char *)data, PAGE_SIZE, "No.%d\n", current_data_index);
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
		WARNING("write to file fail.");
		goto exit_save_testing_data;
	...
	for (i = 0; i < sys.sc_sensor_num; i++) {
		...
		for (j = 0; j < sys.sc_driver_num; j++) {
			...
			bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "%d,", tmp);
			if (tmp > max) {
				max = tmp;
			}
			if (tmp < min) {
	...
	if (sys.key_number > 0) {
		bytes = (s32) snprintf((char *)data, PAGE_SIZE, "Key Rawdata:\n");
		for (i = 0; i < sys.key_number; i++) {
			if (_node_in_key_chn(i + sys.sc_sensor_num * sys.sc_driver_num, module_cfg, sys.key_offest) < 0) {
				continue;
			}
			bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "%d,", current_data_temp[i + sys.sc_sensor_num * sys.sc_driver_num]);
		}
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "\n");
		ret = fwrite(data, bytes, 1, fp);
		if (ret < 0) {
	...
	bytes = (s32) snprintf((char *)data, PAGE_SIZE, "  Maximum:%d  Minimum:%d  Average:%d\n\n", max, min, average);
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
		WARNING("write to file fail.");
		goto exit_save_testing_data;
	...
	if ((test_types & _ACCORD_CHECK) != 0) {
		bytes = (s32) snprintf((char *)data, PAGE_SIZE, "Channel_Accord :(%d)\n", FLOAT_AMPLIFIER);
		ret = fwrite(data, bytes, 1, fp);
		if (ret < 0) {
			WARNING("write to file fail.");
			goto exit_save_testing_data;
		...
		for (i = 0; i < sys.sc_sensor_num; i++) {
			bytes = 0;
			for (j = 0; j < sys.sc_driver_num; j++) {
				bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "%ld,", channel_max_accord[i + j * sys.sc_sensor_num]);
			}
			bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "\n");
			ret = fwrite(data, bytes, 1, fp);
			if (ret < 0) {
...
static s32 _save_test_result_data(char *save_test_data_dir, int test_types, u8 *shortresult)
{
	...
	bytes = (s32) snprintf((char *)data, PAGE_SIZE, "Test Result:");
	if (test_error_code == _CHANNEL_PASS) {
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Pass\n\n");
	} else {
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Fail\n\n");
	}
	bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Test items:\n");
	if ((test_types & _MAX_CHECK) != 0) {
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Max Rawdata:  ");
		if (test_error_code & _BEYOND_MAX_LIMIT) {
			bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "NG !\n");
		} else {
			bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "pass\n");
		}
	}
	...
	if ((test_types & _MIN_CHECK) != 0) {
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Min Rawdata:  ");
	...
	if ((test_types & _ACCORD_CHECK) != 0) {
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Area Accord:  ");

		if (test_error_code & _BETWEEN_ACCORD_AND_LINE) {
			bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Fuzzy !\n");
		} else {
	...
	if ((test_types & _OFFSET_CHECK) != 0) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Max Offest:  ");
	...
	if ((test_types & _JITTER_CHECK) != 0) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Max Jitier:  ");
	...
	if (test_types & _UNIFORMITY_CHECK) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Uniformity:  ");
	...
	if ((test_types & _KEY_MAX_CHECK) != 0) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Key Max Rawdata:  ");
	...
	if ((test_types & _KEY_MIN_CHECK) != 0) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Key Min Rawdata:  ");
	...
	if (test_types & (_VER_EQU_CHECK | _VER_GREATER_CHECK | _VER_BETWEEN_CHECK)) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Device Version:  ");
	...
	if (test_types & _MODULE_TYPE_CHECK) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Module Type:  ");
	...
	if ((test_types & _MODULE_SHORT_CHECK) != 0) {
		...
		bytes = (s32) snprintf((char *)data, PAGE_SIZE, "Module short test:  ");
		...
		if (test_error_code & _GT_SHORT) {
			...
			for (index = 0; index < shortresult[0]; index++) {
				...
				if (shortresult[1 + index * 4] & 0x80) {
					...
					bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Drv%d - ", shortresult[1 + index * 4] & 0x7F);
					...
					if (shortresult[1 + index * 4] == (sys.max_driver_num + 1)) {
						...
						bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "GND\\VDD%d - ", shortresult[1 + index * 4] & 0x7F);

					}

					else {

						bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Sen%d - ", shortresult[1 + index * 4] & 0x7F);

					}
				}
				if (shortresult[2 + index * 4] & 0x80) {

					bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Drv%d are shorted", shortresult[2 + index * 4] & 0x7F);
					...
					if (shortresult[2 + index * 4] == (sys.max_driver_num + 1)) {
						...
						bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "GND\\VDD are shorted");

					}

					else {

						bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "Sen%d are shorted", shortresult[2 + index * 4] & 0x7F);

					}
				}
				bytes += (s32) snprintf((char *)&data[bytes], PAGE_SIZE, "(R=%d Kohm)\n", (((shortresult[3 + index * 4] << 8) + shortresult[4 + index * 4]) & 0xffff) / 10);
...
static s32 _init_test_paramters(char *inipath)
{
	...
	u8 sensor_id = ts->sensor_id;
	const u8 *module_cfg_tmp = ts->pdata->test_data[sensor_id];
	u8 cfg_lens = ts->pdata->test_data_len[sensor_id];

	if (cfg_lens < GTP_CONFIG_MIN_LENGTH ||
		cfg_lens > GTP_CONFIG_MAX_LENGTH)
		return -EINVAL;
#endif
...
#else
	memcpy(module_cfg, module_cfg_tmp, GTP_CONFIG_MAX_LENGTH);
