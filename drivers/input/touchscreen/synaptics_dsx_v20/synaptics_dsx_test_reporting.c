...
int factory_get_rawdata_info(struct device *dev, char *buf)
{
	int rx_num = f54->rx_assigned;
	int tx_num = f54->tx_assigned;

	if (f54->rmi4_data->board->cap_button_map) {
		if (tx_num > rx_num)
			tx_num--;
		else if (tx_num < rx_num)
			rx_num--;
	}
	return snprintf(buf, PAGE_SIZE,
				"RX:%u TX:%u HIGH:%u LOW:%u\n", rx_num, tx_num, 0, 0);
}
EXPORT_SYMBOL(factory_get_rawdata_info);

static int factory_test(struct device *dev, unsigned char report_type)
{
...
int factory_get_rawdata(struct device *dev, char *buf)
{
	...
	int retval = -1;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	/* Get Rx/Tx Electrodes */
	rx_num = f54->rx_assigned;
	tx_num = f54->tx_assigned;

	retval = factory_test(dev, F54_RAW_16BIT_IMAGE);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: read report data is fail.\n",
				__func__);
		return retval;
	}
	/* Print data report */
	pr_notice("%s: Print data report\n", __func__);
	if (!f54->report_data) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: report data is null.\n",
				__func__);
		retval = -ENOMEM;
		return retval;
	} else{
		...
		for (ii = 0; ii < tx_num; ii++) {
			...
			if ((f54->rmi4_data->board->cap_button_map) && (tx_num < rx_num))
				report_data_16++;
		...
		/* Do reset */
		pr_notice("%s: Do reset\n", __func__);
		if (gpio_is_valid(f54->rmi4_data->board->reset_gpio)) {
			gpio_set_value(f54->rmi4_data->board->reset_gpio, 0);
			usleep(RMI4_GPIO_SLEEP_LOW_US);
			gpio_set_value(f54->rmi4_data->board->reset_gpio, 1);
			msleep(f54->rmi4_data->board->reset_delay);
		} else
...
EXPORT_SYMBOL(factory_get_rawdata);

int factory_short_test(struct device *dev, char *buf)
{
	int retval = -1;
	int ii;
	int jj;
	...
	retval = factory_test(dev, F54_TX_TO_TX_SHORT);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: read report data is fail.\n",
				__func__);
	...
	retval = factory_test(dev, F54_RX_TO_RX1);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: read report data is fail.\n",
				__func__);
	...
	retval = factory_test(dev, F54_RX_TO_RX2);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: read report data is fail.\n",
				__func__);
...
EXPORT_SYMBOL(factory_short_test);

int factory_get_calibration_ret(struct device *dev)
{
	return true;
}
EXPORT_SYMBOL(factory_get_calibration_ret);

int factory_proc_hibernate_test(struct device *dev)
{
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	int retval;
	char number_of_re[2];
	if (gpio_is_valid(rmi4_data->board->reset_gpio)) {
		...
		if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev,
					"%s: can't read i2c after reset.\n",
					__func__);
			goto error_reset;
		}
	} else
		dev_warn(&rmi4_data->i2c_client->dev,
				"%s: has no reset gpio, so pass.\n",
				__func__);
...
EXPORT_SYMBOL(factory_proc_hibernate_test);
