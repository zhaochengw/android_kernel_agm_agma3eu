...
#ifdef CONFIG_TP_TYPE_SWITCH
int ctp_enabled = 1;
extern int set_emtp_enable_switch(struct device *dev, bool enable);
extern bool get_emtp_enable_switch(struct device *dev);
#endif /* CONFIG_TP_TYPE_SWITCH */

static int ts_debug_mask;
module_param_named(debug_mask, ts_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);
...
static u8 asic_to_hex(unsigned char val);
static int synaptics_pinctrl_init(struct synaptics_rmi4_data *rmi4_data);
static int synaptics_pinctrl_select(struct synaptics_rmi4_data *rmi4_data, bool on);
...
static int synaptics_rmi4_parse_dt(struct device *dev,
				struct synaptics_dsx_platform_data *rmi4_pdata)
{
	struct device_node *np = dev->of_node;
	struct property *prop;
	...
	rc = of_property_read_u32(np, "synaptics,fw-image-version-offset", &temp_val);
	if (!rc)
		rmi4_pdata->fw_image_version_offset = temp_val;
	else if (rc != -EINVAL)
		dev_err(dev, "Unable to read fw image version offset\n");
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!pdata->cap_button_map) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: cap_button_map is not in use\n",
				__func__);
		return 0;
...
flash_prog_mode:
	...
	module_id1 = asic_to_hex(rmi->product_id_string[8]);
	module_id2 = asic_to_hex(rmi->product_id_string[9]);
	rmi4_data->module_id = (module_id1 << 4) | module_id2;
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	input_set_abs_params(rmi4_data->input_dev,
			ABS_MT_POSITION_X, 0,
			platform_data->disp_maxx, 0, 0);
	input_set_abs_params(rmi4_data->input_dev,
			ABS_MT_POSITION_Y, 0,
			platform_data->disp_maxy, 0, 0);
#ifdef REPORT_2D_W
	input_set_abs_params(rmi4_data->input_dev,
...
#ifdef CONFIG_TP_TYPE_SWITCH
static int set_ctp_enable_switch(struct device *dev, bool enable)
{
	int retval = 0;

	if (enable) {
		ctp_enabled = 1;
		retval = synaptics_rmi4_resume(dev);
		if (retval) {
			dev_err(dev, "failed to enter active power mode\n");
		}

	} else {
		retval = synaptics_rmi4_suspend(dev);
		if (retval) {
...
static bool get_ctp_enable_switch(struct device *dev)
{
	return ctp_enabled;
}
#endif /* CONFIG_TP_TYPE_SWITCH */

#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_GLOVE
static int set_glove_switch(struct device *dev, bool enable)
{
	int retval = 0;
	unsigned char glove_ctrl;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);

	if (enable) {
		glove_ctrl = (glove_ctrl & ~MASK_2BIT);
		glove_ctrl = (glove_ctrl | 0x11);
	} else {
		glove_ctrl = (glove_ctrl & ~MASK_2BIT);
	}
	...
	if (retval < 0)
		dev_err(&(rmi4_data->input_dev->dev),
				"%s: Failed to %s glove mode\n",
				__func__, enable ? "open" : "close");
	else
		rmi4_data->glove_enabled = enable;
...
static bool get_glove_switch(struct device *dev)
{
	...
	return rmi4_data->glove_enabled;
}
#endif /* CONFIG_TOUCHSCREEN_SYNAPTICS_GLOVE */
...
static int factory_ts_func_test_register(struct synaptics_rmi4_data *data)
{
	data->ts_test_dev.dev = &data->i2c_client->dev;
...
#ifdef CONFIG_TP_TYPE_SWITCH
	data->ts_test_dev.get_ctp_enable_switch = get_ctp_enable_switch;
	data->ts_test_dev.set_ctp_enable_switch = set_ctp_enable_switch;
	data->ts_test_dev.get_emtp_enable_switch = get_emtp_enable_switch;
	data->ts_test_dev.set_emtp_enable_switch = set_emtp_enable_switch;
#endif /* CONFIG_TP_TYPE_SWITCH */
		...
		const struct i2c_device_id *dev_id)
{
	...
	if (!retval && rmi4_data->ts_pinctrl) {
		retval = synaptics_pinctrl_select(rmi4_data, true);
		if (retval < 0)
	...
	factory_ts_func_test_register(rmi4_data);
...
exit_free_client_data:
	if (rmi4_data->ts_pinctrl) {
		if (synaptics_pinctrl_select(rmi4_data, false) < 0)
...
static int synaptics_rmi4_remove(struct i2c_client *client)
{
	...
	if (rmi4_data->ts_pinctrl) {
		ret = synaptics_pinctrl_select(rmi4_data, false);
		if (ret < 0)
...
static int synaptics_pinctrl_select(struct synaptics_rmi4_data *rmi4_data,
						bool on)
{
