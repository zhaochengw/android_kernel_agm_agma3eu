...
int factory_get_fs_fw_version(struct device *dev, char *buf)
{
	...
	#if defined(CONFIG_TOUCHSCREEN_GT9XX_COMPATIBLE_MODE) 
	data->fw_version = (gtp_default_FW_fl[12]<<8) + gtp_default_FW_fl[13];
...
s32 gup_clk_calibration(void)
{
    ...
    {
		GTP_DEBUG("CLK calibration DAC %d", i);
...
static u8 gup_download_fw_dsp(struct i2c_client *client, u8 dwn_mode)
{
    ...
    {
		GTP_DEBUG("[download_fw_dsp]Begin esd check dsp fw---->>");
        ...
        //if(FAIL == ret)
        {
			...
			GTP_DEBUG("esd recovery redownload firmware dsp code");
...
static u8 gup_download_fw_ss51(struct i2c_client *client, u8 dwn_mode)
{
    ...
    if (GTP_FL_FW_BURN == dwn_mode)
    {
		GTP_DEBUG("download firmware ss51");
    }
    else
    {
		GTP_DEBUG("check firmware ss51");
    }    
...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
    ...
    {
		GTP_DEBUG("Search for GT9XXF firmware file to update");
        ...
        for (i = 0; i < GUP_SEARCH_FILE_TIMES; ++i)
        {
            if (0 == searching_file)
            {
				GTP_DEBUG("Force terminate auto update for GT9XXF...");
        ...
        if (i == 50)
        {
			GTP_DEBUG("Search timeout, update aborted");
            return FAIL;
        }
        else
        {
...
s32 gup_fw_download_proc(void *dir, u8 dwn_mode)
{
    ...
    if (NULL == dir)
    {
        ...
        {
			GTP_DEBUG("[fw_download_proc]Begin fw download ......");
        }
        else if (GTP_FL_ESD_RECOVERY == dwn_mode)       // GTP_FL_ESD_RECOVERY: GT9XXF esd recovery mode
        {
			GTP_DEBUG("[fw_download_proc]Begin fw esd recovery check ......");
        }       
        else
        {
			GTP_DEBUG("[fw_download_proc]Being fw repair check......");
        }
    }  
    else
    {
		GTP_DEBUG("[fw_download_proc]Begin firmware update by bin file");
    }  
