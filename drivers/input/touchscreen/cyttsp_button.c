...
struct cyttsp_button_data {
	...
	u16 fw_ic_version;
	u16 fw_fs_version;
	bool button_wake_state;
	struct pinctrl *ts_pinctrl;
	struct pinctrl_state *gpio_state_active;
	struct pinctrl_state *gpio_state_suspend;
};
static int button_debug_mask;

module_param_named(button_debug_mask, button_debug_mask, int, S_IRUGO | S_IWUSR);

#undef dev_info

#define dev_info(dev, format, arg...) do { if (button_debug_mask) {dev_printk(KERN_INFO , dev , format , ## arg); } } while (0)

static struct i2c_client *global_client;
static int probe_flag = 2;
			...
			struct cyttsp_button_platform_data *pdata)
{
	...
	if (pdata->config_array_size != 0) {
		...
		ret = of_property_read_u32(np, "cyttsp,panel-id", &temp_val);
		if (ret) {
		...
		ret = of_property_read_string(np, "cyttsp,fw-name",
						&info->fw_name);
		if (ret && (ret != -EINVAL)) {
...
static int cyttsp_button_input_enable(struct input_dev *in_dev)
{
	...
	if (data->button_wake_state== true)
		return 0;
...
static int cyttsp_button_input_disable(struct input_dev *in_dev)
{
	...
	if (data->button_wake_state == false)
		return 0;
...
static irqreturn_t cyttsp_button_interrupt(int irq, void *dev_id)
{
	...
	dev_info(dev, "In cyttsp buttun interrupt.\n");
	if (data->enable) {
		ret = cyttsp_read_reg(data, CYTTSP_REG_TOUCHMODE);
		if (ret < 0) {
			dev_err(&data->client->dev, "Failed to read touch mode reg\n");
			return IRQ_NONE;
		} else {
			mutex_lock(&data->input_dev->mutex);
			data->glove_mode = !!(ret & (1 << CYTTSP_GLOVE_MODE_SHIFT));
			mutex_unlock(&data->input_dev->mutex);
		}

		ret = cyttsp_read_reg(data, pdata->button_status_reg);
		if (ret < 0) {
			dev_err(&data->client->dev, "Failed to read status!\n");
			return IRQ_NONE;
		}

		keystates = (unsigned long)ret;
		if (keystates == 3)
			return IRQ_NONE;
...
static ssize_t cyttsp_firmware_update_store(struct kobject *obj,
	struct kobj_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp_button_data *data = i2c_get_clientdata(global_client);
	int i;

	if (sscanf(buf, "%u", &i) == 1)
		if (i == 1)
			cyttsp_firmware_update(data, update_flag);

	return size;
}

static ssize_t cyttsp_debug_enable_show(struct kobject *obj,
...
static ssize_t cyttsp_debug_enable_store(struct kobject *obj,
	struct kobj_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp_button_data *data = i2c_get_clientdata(global_client);
	struct device *dev = &data->client->dev;
...
static ssize_t cyttsp_fw_ic_version_show(struct kobject *obj,
	struct kobj_attribute *attr, char *buf)
{
	struct cyttsp_button_data *data = i2c_get_clientdata(global_client);
	struct device *dev = &data->client->dev;
	...
	data->fw_ic_version = in_chip_id[1] + (in_chip_id[0] << 8);

	return snprintf(buf, PAGE_SIZE, "%d\n", data->fw_ic_version);
}

static ssize_t cyttsp_fw_fs_version_show(struct kobject *obj,
	struct kobj_attribute *attr, char *buf)
{
	struct cyttsp_button_data *data = i2c_get_clientdata(global_client);
	struct device *dev = &data->client->dev;
	...
	cyttsp_get_lockdown_info(data, false);

	fw_name = cyttsp_get_fw_name(data);
	if (fw_name == NULL) {
		dev_err(dev, "fw_name is empty!\n");
		return snprintf(buf, PAGE_SIZE, "fw_name is empty!\n");
	}
	ret = request_firmware(&fw, fw_name, dev);
	if (ret < 0) {
		dev_err(dev, "Failed to request firmware %s\n", fw_name);
		return snprintf(buf, PAGE_SIZE, "Request firmware failed!\n");
	}
	...
	data->fw_fs_version = id[1] + (id[0] << 8);
	return snprintf(buf, PAGE_SIZE, "%d\n", data->fw_fs_version);
}

static struct kobj_attribute firmware_update_attr = {
	.attr = {
		.name = "fwupdate",
		.mode = S_IWUSR,
	},
	.store = cyttsp_firmware_update_store,
};

static struct kobj_attribute debug_enable_attr = {
	.attr = {
		.name = "debug_enable",
		.mode = S_IWUSR | S_IRUSR,
	},
	.show = cyttsp_debug_enable_show,
	.store = cyttsp_debug_enable_store,
};
...
static struct kobj_attribute fw_ic_version_attr = {
	...
	.show = cyttsp_fw_ic_version_show,
};
...
static struct kobj_attribute fw_fs_version_attr = {
	...
	.show = cyttsp_fw_fs_version_show,
};

static struct attribute *cyttsp_attrs[] = {
	&firmware_update_attr.attr,
	&debug_enable_attr.attr,
		...
		const struct i2c_device_id *id)
{
	...
	int i;
	u8 val;
	struct kobject *touch_button_obj;
	u8 cmd[2] = {0};
	...
	if (error < 0) {
		...
		error = cyttsp_send_command(data, CYTTSP_CMD_ENTER_BTLD, NULL, 0, 0);
		if (error) {
			dev_err(&client->dev, "Failed to enter into bootloader!\n");
			data->client->addr = data->bootloader_addr;
			goto err_free_regulator;
		} else {
		/* Exit bootloader mode */
			cmd[0] = 0;
			error = cyttsp_send_command(data, CYTTSP_CMD_EXIT_BTLD, cmd, 1, 0);
			if (error) {
				dev_err(&client->dev, "Failed to exit bootloader mode !\n");
				data->client->addr = data->app_addr;
				goto err_free_regulator;
			} else {
				update_flag = 1;
	...
	touch_button_obj = kobject_create_and_add("touchbutton", NULL);
	if (!touch_button_obj) {
		error = -EFAULT;
		dev_err(&client->dev, "Failure %d creating touch button obj\n",
			error);
		goto err_create_touchbutton_obj;
	}
	error = sysfs_create_group(touch_button_obj, &cyttsp_attr_group);
	if (error)
...
err_irq_gpio_req:
	...
	if (data->ts_pinctrl) {
		if (cyttsp_button_pinctrl_select(data, false) < 0) {
...
/* Module information */
MODULE_AUTHOR("Robin <shaozuodong@hisense.com>");
