...
static int diag_send_to(char cmd_code, char subsys_id, char cmd_code_hi)
{
	int i = 0;
	if (cmd_code == OEM_CMD_CODE) {
		for (i = 0; i < sizeof(app_sub_cmd_array); i++) {
			if (subsys_id  == app_sub_cmd_array[i])
				return APPS_DATA;
			}
...
int diag_process_apps_pkt(unsigned char *buf, int len)
{
	...
	proc = diag_send_to((char)(entry.cmd_code), (char)(entry.subsys_id), (char)(entry.cmd_code_hi));
