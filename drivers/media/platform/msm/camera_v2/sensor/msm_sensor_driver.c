...
static int32_t msm_sensor_fill_flash_subdevid_by_name(
				struct msm_sensor_ctrl_t *s_ctrl, const char *name)
{
	int32_t rc = 0;
	struct device_node *src_node = NULL;
	...
	*flash_subdev_id = -1;
	...
	if (!src_node) {
		...
		rc = of_property_read_u32(src_node, "cell-index", &val);
		CDBG("%s qcom,led-flash-src cell index %d, rc %d\n", __func__,
			val, rc);
		if (rc < 0) {
			pr_err("%s failed %d\n", __func__, __LINE__);
			return -EINVAL;
...
CSID_TG:
	...
	*  sensor control structure */
	...
	rc = msm_sensor_fill_flash_subdevid_by_name(s_ctrl, slave_info->flash_name);
	if (rc < 0) {
		pr_err("%s failed %d\n", __func__, __LINE__);
		goto free_camera_info;
	}
	...
	CDBG("[berry]%s: is_yuv=%d pos=%d mnt=%d  flag=0x%lx\n",slave_info->sensor_name,
		is_yuv, s_ctrl->sensordata->sensor_info->position, 
		s_ctrl->sensordata->sensor_info->sensor_mount_angle,
